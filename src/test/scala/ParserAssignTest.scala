...
class AssignLHSTest extends AnyFunSuite {
  ...
  test("Successfully parses ident") {
    assert(assignLHSWhitespace.runParser(" example").contains(Ident("example",(1,2))))
  }
  ...
  test("Successfully parses an array-elem") {
    assert(
      assignLHSWhitespace
        .runParser("list[21][2]")
        .contains(ArrayElem(Ident("list",(1,1)), List(IntLiter(21,(1,6)), IntLiter(2,(1,10))),(1,1)))
    )
  }

  test("Successfully parses a pair-elem") {
    assert(assignLHSWhitespace.runParser("fst 10").contains(Fst(IntLiter(10,(1,5)),(1,1))))
    assert(assignLHSWhitespace.runParser("snd x").contains(Snd(Ident("x",(1,5)),(1,1))))
  }
}
  ...
  test("Successfully parses call with one argument") {
    assert(
      assignRHSWhitespace
        ...
        .contains(Call(Ident("_p",(1,9)), Some(ArgList(List(IntLiter(65,(1,13))))),(1,2)))
    )
