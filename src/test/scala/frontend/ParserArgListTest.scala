...
class FrontendParserArgListTest extends AnyFunSuite {
  ...
  test("Successfully parses single element argList") {
    assert(
      argList.runParser("10").contains(ArgList(List(IntLiter(10, (1, 1)))))
    )
    assert(
      argList
        .runParser("'c''")
    ...
    assert(
      argList
        .runParser("10*6")
        .contains(
          ArgList(List(Mul(IntLiter(10, (1, 1)), IntLiter(6, (1, 4)), (1, 3))))
        )
    )
    assert(
      argList
